...
const NINE_B = BigInt('9000000000000000'); // 9 billion USDT in 6-decimal raw
...
export default function FaucetPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setFaucetBal(fb as bigint); setTotal(tc as bigint); setClaimers(cl as bigint);
      if (address) {
        ...
        setUserBal(ub as bigint); setCd(Number(cdSec as bigint));
      }
  ...
  const claim = async () => {
    ...
    try {
      ...
      toast.dismiss();
      setTxHash(tx as `0x${string}`); setSuccess(true); setCd(86400); loadData();
    } catch(e:any){
      toast.dismiss();
    ...
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', background:'#08080F' }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'40px 20px' }}>
        ...
        <div style={{ textAlign:'center', marginBottom:32 }}>
          ...
          <p style={{ fontSize:14, color:'#8888AA', maxWidth:420, lineHeight:1.65 }}>Claim 500 free USDT every 24 hours — use it to buy tokens and make offers on VENDR Market.</p>
        </div>
        ...
        <div style={{ width:'100%', maxWidth:520, marginBottom:26 }}>
          <div style={{ display:'flex', justifyContent:'space-between', fontSize:11, color:'#8888AA', marginBottom:5, fontFamily:'Space Mono,monospace' }}>
            <span>Faucet reserves</span><span style={{ color:'#C8F000' }}>{pct.toFixed(1)}% remaining</span>
          </div>
          <div className="bar" style={{ height:8, borderRadius:4 }}><div className="bar-fill" style={{ width:`${pct}%`, borderRadius:4 }} /></div>
        </div>

        {/* claim card */}
        <div className="card" style={{ width:'100%', maxWidth:400, padding:32, textAlign:'center', borderColor:'rgba(200,240,0,.18)' }}>
          <div style={{ fontSize:13, color:'#8888AA', marginBottom:8 }}>You will receive</div>
          <div style={{ fontFamily:'Boogaloo,cursive', fontSize:56, color:'#C8F000', marginBottom:4 }}>500</div>
          <div style={{ fontSize:18, fontWeight:700, marginBottom:22 }}>USDT</div>

          {isConnected && (
            <div style={{ padding:'10px 14px', background:'rgba(255,255,255,.04)', border:'1px solid rgba(255,255,255,.07)', borderRadius:8, fontSize:12, marginBottom:20 }}>
              <div style={{ color:'#44445A', marginBottom:3, fontFamily:'Space Mono,monospace', fontSize:10, textTransform:'uppercase', letterSpacing:'.06em' }}>Your USDT Balance</div>
              <div style={{ fontFamily:'Space Mono,monospace', fontSize:16, fontWeight:700, color:'#C8F000' }}>{dataLoad?'--':`${fmtU(userBal)} USDT`}</div>
            </div>
          )}

          {cd > 0 ? (
            <>
              <div style={{ padding:16, background:'rgba(245,166,35,.07)', border:'1px solid rgba(245,166,35,.2)', borderRadius:10, marginBottom:16 }}>
                <div style={{ fontSize:11, color:'#F5A623', marginBottom:5 }}>⏱ Next claim available in</div>
                <div style={{ fontFamily:'Space Mono,monospace', fontSize:30, fontWeight:700, color:'#F5A623' }}>{cdStr}</div>
              </div>
              <button className="btn btn-lime" style={{ width:'100%', padding:'13px 0', fontSize:15 }} disabled>Come back later</button>
            </>
          ) : (
            <button className="btn btn-lime" style={{ width:'100%', padding:'13px 0', fontSize:15 }} disabled={loading||!isConnected} onClick={claim}>
              {!isConnected?'Connect Wallet to Claim':loading?<span style={{ display:'flex',alignItems:'center',justifyContent:'center',gap:8 }}><span className="spinner spinner-black"/>Claiming…</span>:'Claim 500 USDT Free →'}
            </button>
          )}

          <div style={{ fontSize:11, color:'#44445A', marginTop:14, lineHeight:1.6 }}>One claim per wallet per 24 hours.<br/>Use USDT to buy tokens and make offers on VENDR.</div>
        </div>
