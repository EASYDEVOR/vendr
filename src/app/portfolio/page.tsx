...
export default function PortfolioPage() {
  ...
  async function cancelOffer(offerId: bigint) {
    ...
    try {
      ...
      toast.dismiss();
      toast.error('Offers can only be cancelled by the seller ignoring them, or automatically when the listing is cancelled.');
    } catch (e: any) { 
      toast.dismiss(); 
      toast.error(e?.shortMessage ?? 'Failed'); 
    }
              ...
              <div key={o.id.toString()} style={{ padding: '14px 16px', background: '#0F0F1C', border: '1px solid rgba(255,255,255,.07)', borderRadius: 10, marginBottom: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button 
                    className="btn btn-ghost" 
                    style={{ flex: 1, padding: '8px 0', fontSize: 12 }}
                    onClick={() => toast("Offer editing is not supported yet. You can cancel and create a new offer.", { icon: 'ℹ️' })}
                  >
                    ✏️ Edit Offer
                  </button>
                  <button 
