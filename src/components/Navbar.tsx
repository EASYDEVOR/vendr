...
const TABS = [
  ...
  { label: 'Social OTC',      path: '/social',    badge: 'SOON', bc: '#F5A623' },
  { label: 'NFT Marketplace', path: '/nft',        badge: 'SOON', bc: '#F5A623' },
  { label: 'Portfolio',       path: '/portfolio',  bold: true },
  { label: 'Faucet',          path: '/faucet',     badge: 'FREE', bc: '#00C805' },
];
...
export default function Navbar() {
  ...
  return (
    <nav style={{
      ...
      height: 58, background: 'rgba(8,8,15,.97)',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 2, flex: 1 }}>
        {TABS.map(t => (
          <button
            key={t.path}
            className={`nav-tab ${path === t.path ? 'active' : ''}`}
            style={{ fontWeight: t.bold ? 800 : 600 }}
            onClick={() => router.push(t.path)}
          >
            {t.label}
            {t.badge && (
              <span style={{
                background: t.bc, color: '#000',
